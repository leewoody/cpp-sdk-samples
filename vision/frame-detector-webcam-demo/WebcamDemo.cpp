...
int main(int argsc, char** argsv) {
    ...
    try {
        ...
        ("data,d", po::wvalue<affdex::path>(&data_dir),
            ...
            ("resolution,r",
             po::value < std::vector < int >
                 > (&resolution)->default_value(DEFAULT_RESOLUTION, "1280 720")->multitoken(),
        ...
        if (sync) {
            ...
            frame_detector =
                std::unique_ptr<vision::Detector>(new vision::FrameDetector(data_dir, process_framerate, num_faces));
        }
